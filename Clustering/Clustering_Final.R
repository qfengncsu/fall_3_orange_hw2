...
#Make clusters
toC1 <- cbind(combined3$avg,combined3$std.ppb,combined3$std.avg_dist)
toC2 <- cbind(combined1$std.lat,combined1$std.lon)


clusters.c <- hclust(dist(toC1),method="complete")
#clusters.c2 <- hclust(dist(toC2),method="complete")
...
#combined3$clus <- cutree(clusters.c2,6) #it looks like 6 clusters is reasonable
...
mean(clu1$count, na.rm=TRUE)
mean(clu2$count, na.rm=TRUE)
mean(clu3$count, na.rm=TRUE)
mean(clu4$count, na.rm=TRUE)
mean(clu5$count, na.rm=TRUE)
mean(clu6$count, na.rm=TRUE)

top12 <- subset(clu6, count==365)
top12 <- as.data.frame(cbind(top12$listing_id, top12$n, top12$sscore, top12$avg_dist, top12$ppb))
...
ggmap(map, fullpage = TRUE) +
  geom_point(data = clu1, aes(x = longitude, y = latitude), color = 'purple', size = 2) +
...
ggmap(map, fullpage = TRUE) +
  geom_point(data = clu,6 aes(x = longitude, y = latitude), color = 'purple', size = 2) +
  geom_point(data = attractions8, aes(x = Lon, y = Lat), color = 'black', size = 5)
